...
This is the only guide you need. Part 0 is one-time setup. Part 1 is all most people ever use.
...
Copying **commands** from this guide into your terminal, and copying **files** into folders, are different things:
...
**Step 1 — Open the folder.** Go to the repo on GitHub, click a subject folder, then `notes/`.
...
**Step 6 — Done.** A maintainer clicks Merge. Your file has history forever — who added it, when, why. Second time onwards it's the same clicks.

> Screenshots above are from the official GitHub Docs (linked under each one), so they always match the current GitHub UI. This repo has no screenshot folder to go stale.
...
`Unit-<n>-<topic>-<your-name>.pdf` — e.g. `Unit-2-Laplace-rahul.pdf`. One topic per file. No `final-final-v2.pdf`. Question papers go in `question-banks/`, lab work in `lab-manuals/`, never `client_secret*.json` / passwords / phone numbers.
